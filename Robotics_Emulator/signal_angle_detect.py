...
def find_phase_2_electric_boogaloo(waveform, freq, cutoff,
                                   order, sampling_freq):
    """
    Compute the phase of the input waveform relative to a fixed reference.

    This function uses emulates the lowpass filtering method.

    Parameters
    ----------
    waveform : 2-tuple of 1D numpy arrays
        A representation of the input waveform. The first entry is a list of
        times and the second entry is a list of corresponding amplitudes.

    freq : float
        The expected frequency of the incoming wave. All other frequencies are
        filtered out.
    ...
    order : int
        The order of the Butterworth lowpass filter.

    sampling_freq : freq
        The sampling frequency of the lowpass filter.

    Returns
    -------
    float
        The computed phase of the input waveform
        (relative to a fixed internal reference).

    """
    # Multiply the input waveform by a cosinusoid at the expected frequency.
    cos_prod = np.array(
        [magnitude * np.cos(2*np.pi*freq*time)
         for time, magnitude in zip(waveform[0], waveform[1])]
    )

    # Multiply the input waveform by a sinusoid at the expected frequency.
    sin_prod = np.array(
        [magnitude * np.sin(2*np.pi*freq*time)
         for time, magnitude in zip(waveform[0], waveform[1])]
    )

    # Obtain parameters for a digit Butterworth lowpass filter with specified
    # order and cutoff frequency.
    sos = signal.butter(
        order, 2 * cutoff / sampling_freq, btype='low',
        analog=False, output='sos', fs=sampling_freq,
    )

    # Apply the Butterworth filter to the multiplied waveforms.
    cos_prod_filt = signal.sosfiltfilt(sos, cos_prod)
    sin_prod_filt = signal.sosfiltfilt(sos, sin_prod)

    # By math, this gives the phase (in degrees).
    return np.arctan(sin_prod_filt / cos_prod_filt)
...
if __name__ == "__main__":
    ...
    # test code for phase_detection_2_electric_boogaloo
    freq = 50000
    cutoff = 4 * freq
    order = 5
    sampling_freq = 10 * freq
    times = np.linspace(0, 1000e-3)
    phase = 0
    waveform = (times, np.cos(2*np.pi*(freq*times+phase)))
    print(find_phase_2_electric_boogaloo(waveform, freq, cutoff,
                                         order, sampling_freq))
